// backend\lib\src\routes\admin_routes.dart
...
Router buildAdminRoutes(AdminHandler handler, TokenService tokens) {
  ...
  router.get(
      '/dashboard', requireAuth(tokens, roles: {'admin'})(handler.dashboard));
  router.get('/users', requireAuth(tokens, roles: {'admin'})(handler.users));
  router.get(
      '/finance', requireAuth(tokens, roles: {'admin'})(handler.finance));
